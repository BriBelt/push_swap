...
/*   Updated: 2023/06/06 16:23:45 by bbeltran         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_hex_mark(unsigned int hexval, int *i, int *printed, char c)
{
	if (hexval == 0)
	{
		pft_putnbr_fd(0, 1, printed);
		(*i) += 2;
	}
	else
	{
		pft_putnbr_hex_fd(hexval, 1, c, printed);
		(*i) += 2;
	}
}
...
{
	if (!ptr)
		pft_putstr_fd("0x0", 1, printed);
	else
	{
		pft_putstr_fd("0x", 1, printed);
		pft_putnbr_hex_fd(ptr, 1, 'x', printed);
	}
	(*i) += 2;
}
...
{
	if (!argstr)
		pft_putstr_fd("(null)", 1, printed);
	else
		pft_putstr_fd(argstr, 1, printed);
	(*i) += 2;
}
...
void	check_rest(const char *format, int *i, va_list *ap, int *printed)
{
	if (format[*i] == '%' && (format[(*i) + 1] == 'x'
		...
		pft_putchar_fd(format[(*i)++], 1, printed);
}
